...
exports.restrict = (...roles) => {
  return (req, res, next) => {
    ...
    if (!roles.includes(req.user.role)) {
      return next(new AppError("you have no permission to performthis action .... "));
    }
    next();
  };
};
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(new AppError("There was an error sendingthe email . Try agai n", 500));
  }
});
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  const checkPassword = user.correctPassword(req.body.passwordConfirm, user.password);
  if (!checkPassword) {
